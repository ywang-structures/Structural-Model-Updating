...
for i = 1 : n_modes
    ...
    for j = 1 : n_alpha
        ...
        if (MAT_version >= 2017)
            ...
            if ~isempty(dgsB)
                % use LU reordering
                dPsi_dAlpha_j(P_i) = qq*(U \ (L \ (pp*(dgsB\b_j))));
            else
                dPsi_dAlpha_j(P_i) = U \ (L \ b_j(pp));
            end
        end
        ...
        for k = 1 : n_alpha
            ...
            b_k = b_k(P_i);
            if (MAT_version >= 2017)
                dPsi_dAlpha_k(P_i) = dcompB \ b_k;
            else
                if ~isempty(dgsB)
                    % use LU reordering
                    dPsi_dAlpha_k(P_i) = qq*(U \ (L \ (pp*(dgsB\b_k))));
                else
                    dPsi_dAlpha_k(P_i) = U \ (L \ b_k(pp));
                end
            end
            % The q_i-th entry of dPsi_m remains as 0 from initialization           
            ...
            if (MAT_version >= 2017)
                ...
                if ~isempty(dgsB)
                    % use LU reordering
                    ddPsi_dAlpha_j_dAlpha_k(P_i) = qq*(U \ (L \ (pp*(dgsB\b_jk))));
                else
                    ddPsi_dAlpha_j_dAlpha_k(P_i) = U \ (L \ b_jk(pp));
                end
            end
            for l = 1 : length(Q_i)
...
>>>>>>> 2b641e4ef0e77f30e9f9c519115961b46f3cc12a
=======
function [hess] = HessMPD(x, structModel, expModes, updatingOpts)
% function [hess] = HesMPD(x, structModel, expModes, updatingOpts)
%
...
>>>>>>> 5f6ca81e2d3c4f31160925b5b182ca2386f5aa09
