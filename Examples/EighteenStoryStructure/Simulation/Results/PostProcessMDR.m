...
%% Plot
filename = 'EighteenStoryStructureSim_form3_JACon_Levenberg-Marquardt';
load(filename);
fval1 = fval;
[~,idMin] = min(fval);
...
filename = 'EighteenStoryStructureSim_form3_JACon_Trust-region-reflective';
load(filename);
fval2 = fval;
[~,idMin] = min(fval);
...
lgd = legend('Levenberg-Marquardt','Trust-region-reflective');
set(lgd,'fontsize',FZ);
set(gca,'fontsize',FZ);
