...
errorEigVec(2,:) = abs(x_sort(:,1) - alpha_act) ./ (ones(n_alpha,1) + alpha_act) * 100;
for i = 1:num_star
...
n_alpha = size(alpha,1);
errorEigVec(1,:) = abs(x_sort(:,1) - alpha_act) ./ (ones(n_alpha,1) + alpha_act) * 100;
for i = 1:num_star
...
set(lgd,'fontsize',FZ);
set(gca,'fontsize',FZ);
text(idxLocal-4,mean_errorA(idxLocal),'B','Fontsize',12,'Color','r')
ylim([0 63])
...
text(0.01,0.0005,'B','Fontsize',12,'Color','k')
text(0.65,10^-16,'Global optimum','Fontsize',12,'Color','k')